...
const supportedRemixEsbuildConfigKeys = ["appDirectory", "future", "ignoredRouteFiles", "publicPath", "routes", "serverModuleFormat"];
const SERVER_ONLY_ROUTE_EXPORTS = ["loader", "action", "headers"];
const CLIENT_ROUTE_EXPORTS = ["clientAction", "clientLoader", "default", "ErrorBoundary", "handle", "HydrateFallback", "links", "meta", "shouldRevalidate"];
...
const remixVitePlugin = (remixUserConfig = {}) => {
  ...
  let updateRemixPluginContext = async () => {
    var _viteUserConfig$build, _getServerBundleBuild;
    let remixConfigPresets = (await Promise.all((remixUserConfig.presets ?? []).map(async preset => {
      ...
      let remixConfigPreset = omit__default["default"](await preset.remixConfig(), excludedRemixConfigPresetKeys);
    ...
    if (isSpaMode && serverBundles) {
      console.warn(pc__default["default"].yellow(pc__default["default"].bold("⚠️  SPA Mode: ") + "the `serverBundles` config is invalid with " + "`unstable_ssr:false` and will be ignored`"));
  ...
  let getBrowserManifestForDev = async () => {
    ...
    for (let [key, route] of Object.entries(ctx.remixConfig.routes)) {
      ...
      routes[key] = {
        ...
        index: route.index,
        caseSensitive: route.caseSensitive,
        module: `${resolveFileUrl.resolveFileUrl(ctx, resolveRelativeRouteFilePath(route, ctx.remixConfig))}${CLIENT_ROUTE_QUERY_STRING}`,
        hasAction: sourceExports.includes("action"),
        hasLoader: sourceExports.includes("loader"),
        hasClientAction: sourceExports.includes("clientAction"),
        hasClientLoader: sourceExports.includes("clientLoader"),
    ...
    return {
      ...
      entry: {
        module: resolveFileUrl.resolveFileUrl(ctx, ctx.entryClientFilePath),
        imports: []
      },
      routes
    };
    ...
    writeBundle: {
      ...
      async handler() {
        ...
        if (!ctx.remixConfig.unstable_ssr) {
          await handleSpaMode(serverBuildDirectory, ctx.remixConfig.serverBuildFile, clientBuildDirectory, viteConfig);
        }
      }
    },
    async buildEnd() {
...
function addRefreshWrapper(remixConfig, code, id) {
  ...
  let acceptExports = route || id.endsWith(CLIENT_ROUTE_QUERY_STRING) ? ["clientAction", "clientLoader", "handle", "meta", "links", "shouldRevalidate"] : [];
...
async function getRouteMetadata(ctx, viteChildCompiler, route, readRouteFile) {
  ...
  let info = {
    ...
    index: route.index,
    caseSensitive: route.caseSensitive,
    url: "/" + path__namespace.relative(ctx.rootDirectory, resolveRelativeRouteFilePath(route, ctx.remixConfig)),
    module: `${resolveFileUrl.resolveFileUrl(ctx, resolveRelativeRouteFilePath(route, ctx.remixConfig))}?import`,
...
async function handleSpaMode(serverBuildDirectoryPath, serverBuildFile, clientBuildDirectory, viteConfig) {
